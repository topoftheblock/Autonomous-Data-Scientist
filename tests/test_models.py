...
from agent.nodes import ingestion_node, execution_step, planner_node, replan_node, reporter_node
...
# Ingestion node
# -------------------------------------------------------------------
@patch("agent.nodes.ingestion_node.load_csv")
...
def test_executor_step_success(base_state, monkeypatch):
    ...
    monkeypatch.setattr(
        execution_step, "AgentExecutor", lambda *args, **kwargs: mock_executor
    )

    result = execution_step.execute_step(base_state)
    assert result["error"] == ""
...
def test_executor_step_failure(base_state, monkeypatch):
    ...
    assert "current_step_index" not in result  # not incremented
...
# Report node
# -------------------------------------------------------------------
def test_report_node(base_state, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    base_state["step_results"] = ["Step 1: Profiled age", "Step 2: Cleaned data"]
    result = reporter_node.reporter_node(base_state)
    assert "# Data Science Report" in result["final_report"]
